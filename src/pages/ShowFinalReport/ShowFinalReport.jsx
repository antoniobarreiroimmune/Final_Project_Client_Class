...
import { Box, Text, VStack, Grid, GridItem, Button, HStack, Textarea } from "@chakra-ui/react";
            ...
            <Box maxW="80%" margin="auto" mt={5}>
                ...
                <VStack align="stretch" spacing={4}>
                    ...
                    <Grid templateColumns="repeat(4, 1fr)" gap={6}>
                        <GridItem colSpan={1}>
                            <Text>
                                <strong>Nombre:</strong> {finalReport.name}
                            </Text>
                        </GridItem>
                        <GridItem colSpan={1}>
                            <Text>
                                <strong>Primer nombre:</strong> {finalReport.firstName}
                            </Text>
                        </GridItem>
                        <GridItem colSpan={1}>
                            <Text>
                                <strong>Apellido:</strong> {finalReport.lastName}
                            </Text>
                        </GridItem>
                        <GridItem colSpan={1}>
                            <Text>
                                <strong>DNI:</strong> {finalReport.dni}
                            </Text>
                        </GridItem>
                    </Grid>
                    <HStack justify="space-between">
                        <Text>
                            <strong>Ubicación:</strong> {finalReport.location}
                        </Text>
                        <Text>
                            <strong>Observaciones:</strong> {finalReport.observations}
                        </Text>
                    </HStack>
                    <HStack justify="space-between">
                        <Text>
                            <strong>Violencia de género:</strong>{" "}
                            {finalReport.isGenderViolence ? "Sí" : "No"}
                        </Text>
                        <Text>
                            <strong>Violencia doméstica:</strong>{" "}
                            {finalReport.isDomesticViolence ? "Sí" : "No"}
                        </Text>
                        <Text>
                            <strong>Órgano judicial:</strong> {finalReport.judicialBody}
                        </Text>
                    </HStack>
                    <Box>
                        <Text mb={2}>
                    ...
                    <HStack justify="space-between">
                        ...
                        <Text>
                            ...
                            {new Date(finalReport.updatedAt).toLocaleDateString()}
                        </Text>
                    </HStack>
                    <Text>
                        <strong>Patología completada:</strong>{" "}
                        {finalReport.finalReportCompleted ? "Sí" : "No"}
                    </Text>
